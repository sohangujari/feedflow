...
export async function GET(req: NextRequest) {
  ...
  // Parse query params with some strict defaults for "latest"
  const params: NewsQueryParams = {
    ...
    sortBy:   "publishedAt", // Force sorting by newest
  ...
  try {
    ...
    return withRateLimitHeaders(
      NextResponse.json(response, {
        headers: {
          "Cache-Control": "public, s-maxage=30, stale-while-revalidate=120", // Shorter cache for "latest"
        },
      }),
      rateLimit.headers
    );
  } catch (err) {
    console.error("[/api/news/latest] Error:", err);
    return withRateLimitHeaders(
      NextResponse.json(
